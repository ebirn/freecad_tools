...
logging.basicConfig(
    level=logging.INFO,
    format='lib3mf_utils - %(levelname)s - %(message)s'
)
...
def convert_stl_to_lib3mf_mesh(stl_file_path: str, mesh_object) -> None:
    ...
    try:
        with open(stl_path, 'rb') as f:
            ...
            logger.info(
                f"STL conversion complete: {vertex_count} vertices, "
                f"{len(triangle_data)} triangles"
            )
...
def main():
    ...
    if command == 'create':
        ...
        for i, stl_path in enumerate(sys.argv[3:]):
